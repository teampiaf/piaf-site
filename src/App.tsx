...
export function App() {
  ...
  // COOKIES (préférences)
  // -----------------------------
  const [cookiePrefs, setCookiePrefsState] = useState<CookiePreferences | null>(null);

  useEffect(() => {
    setCookiePrefsState(getCookiePreferences());
  }, []);
  ...
  if (path === "/download") {
    page = <Download />;
  } else if (path === "/projet" || path === "/project" || path === "/decouvrir-le-projet") {
    page = <ProjectPage />;
  } else if (path === "/comment-ca-marche" || path === "/how-it-works") {
    page = <HowItWorksPage />;
  } else if (path === "/mentions-legales" || path === "/mentions") {
  ...
  // ✅ Toujours afficher la page + le bandeau si pas de préférence encore
  return (
    <>
      {page}
